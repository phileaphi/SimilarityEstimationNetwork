...
from .encoder_cores.encoder_core import EncoderCore
...
class GaussianEncoder(Encoder):
    ...
    def _CHECK(cls, core: EncoderCore, prior_flow: Flow, posterior_flow: Flow):
        ...
        if prior_flow is not None:
            try:
                ...
                raise ValueError(
                    'core output size %s does not match prior input size %s' %
                    (core.output_size(), prior_flow.input_size())
                )
        ...
        if posterior_flow is not None:
            try:
                ...
                raise ValueError(
                    'core output size %s does not match posterior input size %s' %
                    (core.output_size(), posterior_flow.input_size())
                )
    ...
    def initialize(self, x, init_scale=1.0):
        ...
        if self.posterior_flow is not None:
            ...
            z, logdet = self.posterior_flow.forward(
                z, init=True, init_scale=init_scale * 0.1
            )
            # [batch, z_shape]
            z = z.view(mu.size())
        else:
    ...
    def sample_from_posterior(self, x, nsamples=1):
        ...
        if self.posterior_flow is not None:
            ...
            z, logdet = self.posterior_flow.forward(
                z_normal.view(z_size[0] * z_size[1], *self.posterior_flow.input_size())
            )
            # [batch, nsamples, z_shape]
            z = z.view(z_size)
    ...
    def sample_from_proir(self, nsamples=1, device=torch.device('cpu')):
        ...
        if self.prior_flow is not None:
            # [nsamples, flow_shape]
            z, logdet = self.prior_flow.forward(
                z_normal.view(nsamples, *self.prior_flow.input_size())
            )
            # [nsamples, z_shape]
    ...
    def _postKL(self, mu, logvar):
        ...
        PostKL_std = (
            PostKL.view(Eye.size() + (-1, )).sum(dim=2) + Eye * PostKL_mean.sum()
        ).std() * dd
    ...
    def encode(self, x, nsamples):
        ...
        if self.prior_flow is None and self.posterior_flow is None:
            ...
            KL = 0.5 * (mu.pow(2) + logvar.exp() - logvar - 1).view(z_size[0],
                                                                    -1).sum(dim=1)
        else:
            # [batch * nsamples] --> [batch, nsamples]
            log_probs_prior = self.log_probability_prior(
                z.view(z_size[0] * z_size[1], *z_size[2:])
            ).view(z_size[0], z_size[1])
            log_probs_posterior = self.log_probability_posterior(
                x, z, distr_params=distr_params
            )
    ...
    def log_probability_posterior(self, x, z, distr_params=None):
        ...
        if distr_params is None:
            ...
            if self.posterior_flow is None:
                ...
                z_normal, logdet = self.posterior_flow.backward(
                    z.view(z_size[0] * z_size[1], *self.posterior_flow.input_size())
                )
                # [batch, nsamples, flow_shape], [batch, nsamples]
        ...
        # [batch, nsamples, z_shape]
        log_probs = logvar.unsqueeze(1) + (z_normal - mu.unsqueeze(1)
                                          ).pow(2).div(logvar.exp().unsqueeze(1) +
                                                       eps) + math.log(math.pi * 2.)
    ...
    def from_params(cls, params: Dict) -> "GaussianEncoder":
        ...
        if posterior_flow_params is not None:
            posterior_flow = Flow.by_name(posterior_flow_params.pop('type')
                                         ).from_params(posterior_flow_params)

        prior_flow = None
        prior_flow_params = params.pop('prior_flow', None)
        if prior_flow_params is not None:
            prior_flow = Flow.by_name(prior_flow_params.pop('type')
                                     ).from_params(prior_flow_params)

        return GaussianEncoder(
            core, prior_flow=prior_flow, posterior_flow=posterior_flow, **params
        )
