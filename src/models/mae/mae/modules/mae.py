...
class MAE(nn.Module):
    ...
    def _CHECK(cls, encoder: Encoder, decoder: Decoder):
        ...
        try:
            ...
            raise ValueError(
                'encoder latent z shape %s does not match decoder input shape %s' %
                (encoder.z_shape(), decoder.z_shape())
            )
    ...
    def loss(self, x, nsamples, eta=0.0, gamma=0.0, free_bits=0.0):
        ...
        reconstruct_err = self.decoder.reconstruct_error(
            x, z.view(z.size(0), z.size(1), *z_shape_dec)
        )
        ...
        loss = recon.mean() + KL.mean().clamp(
            min=free_bits
        ) + loss_postKL_mean + loss_postKL_std
    ...
    def initialize(self, x, init_scale=1.0):
        ...
        z_shape_dec = self.decoder.z_shape()
        return self.decoder.initialize(
            x, z.view(z.size(0), *z_shape_dec), init_scale=init_scale
        )
    ...
    def nll(self, x, k):
        ...
        log_probs_prior = self.encoder.log_probability_prior(z.view(batch * k,
                                                                    *z_size)).view(
                                                                        batch, k
                                                                    )  # noqa
        # [batch, k]
        log_probs_posterior = self.encoder.log_probability_posterior(
