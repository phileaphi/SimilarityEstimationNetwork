...
from ...networks.auto_regressives.made import MADE
from .af import AF
...
class AFMADEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        index = range(self._input_size
                     ) if self.order == 'A' else reversed(range(self._input_size))
        for i in index:
            mu = self.mu(y)
            if self.logvar:
...
class AFMADEDualBlock(nn.Module):
    def __init__(self, input_size, num_hiddens, hidden_size, bias=True, var=True):
        ...
        self.fwd = AFMADEBlock(
            input_size, num_hiddens, hidden_size, 'A', bias=bias, var=var
        )
        self.bwd = AFMADEBlock(
            input_size, num_hiddens, hidden_size, 'B', bias=bias, var=var
        )
...
class AFMADE(AF):
    def __init__(
        ...
        for i in range(num_blocks):
            block = AFMADEDualBlock(
                input_size, num_hiddens, hidden_size, bias=bias, var=var
            )
    ...
    @overrides
    def forward(self, x: torch.Tensor, init=False,
    ...
    @overrides
    def backward(self, y: torch.Tensor, init=False,
                 init_scale=1.0) -> Tuple[torch.Tensor, torch.Tensor]:
        logdet_accum = y.new_zeros(y.size(0))
        for block in self.blocks:
            y, logdet = block.initialize(y, init_scale=init_scale
                                        ) if init else block.backward(y)
            logdet_accum = logdet_accum + logdet
