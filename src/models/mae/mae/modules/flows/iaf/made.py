...
class IAFMADE(IAF):
    def __init__(
        ...
        for i in range(num_blocks):
            block = IAFMADEDualBlock(
                self.nz, num_hiddens, hidden_size, bias=bias, var=var
            )
    ...
    def forward(self, x: torch.Tensor, init=False,
        ...
        for block in self.blocks:
            x, logdet = block.initialize(x, init_scale=init_scale
                                        ) if init else block.forward(x)
