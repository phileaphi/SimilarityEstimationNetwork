...
def deconv3x3(in_planes, out_planes, stride=1, output_padding=0):
    ...
    return ConvTranspose2dWeightNorm(
        in_planes,
        out_planes,
        kernel_size=3,
        stride=stride,
...
class ResNetBlock(nn.Module):
    ...
    def initialize(self, x, init_scale=1.0):
        residual = x if self.downsample is None else self.downsample.initialize(
            x, init_scale=init_scale
        )

        # [batch, planes, ceil(h/stride), ceil(w/stride)]
        out = self.conv1.initialize(x, init_scale=init_scale)
        out = self.activation(out)

        # [batch, planes, ceil(h/stride), ceil(w/stride)]
        out = self.conv2.initialize(out, init_scale=init_scale)
...
class DeResNetBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, output_padding=0):
        ...
        downsample = None
        if stride != 1 or inplanes != planes:
            downsample = ConvTranspose2dWeightNorm(
                inplanes,
                planes,
                kernel_size=1,
                stride=stride,
    ...
    def initialize(self, x, init_scale=1.0):
        ...
        # [batch, planes, h * stride - stride + 1 + op, w * stride - stride + 1 + op]
        out = self.deconv1.initialize(x, init_scale=init_scale)
        out = self.activation(out)

        # [batch, planes, h * stride - stride + 1 + op, w * stride - stride + 1 + op]
        out = self.deconv2.initialize(out, init_scale=init_scale)
        # [batch, planes, ceil(h/stride), ceil(w/stride)]
        return out + residual
...
class DeResNet(nn.Module):
    def __init__(self, inplanes, planes, strides, output_paddings):
        ...
        for i in range(len(planes)):
            ...
            block = DeResNetBlock(
                inplanes, plane, stride=stride, output_padding=output_padding
            )
            blocks.append(block)
            inplanes = plane
