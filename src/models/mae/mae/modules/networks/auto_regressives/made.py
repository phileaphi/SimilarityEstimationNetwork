...
from ..masked import MaskedLinear, MaskedConv2d
...
class MADE(nn.Module):
    ...
    def __init__(self, input_size, num_hiddens, hidden_size, order, bias=True):
        ...
        self.input_layer = MaskedLinear(
            input_size, hidden_size, ('input-hidden', order), total_units, bias=bias
        )
        self.direct_connect = MaskedLinear(
            input_size, input_size, ('input-output', order), total_units, bias=bias
        )

        max_units = self.input_layer.max_units
        self.hidden_layers = []
        for hid in range(1, num_hiddens):
            hidden_layer = MaskedLinear(
                hidden_size,
                hidden_size, ('hidden-hidden', order),
                total_units,
                max_units=max_units,
                bias=bias
            )
        ...
        self.output_layer = MaskedLinear(
            hidden_size,
            input_size, ('hidden-output', order),
            total_units,
            max_units=max_units,
            bias=bias
        )
...
class MADE2d(nn.Module):
    ...
    def __init__(
        ...
        self.direct_connect = MaskedConv2d(
            in_channels,
            in_channels,
            kernel_size,
            mask_type='A',
