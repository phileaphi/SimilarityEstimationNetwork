...
class MaskedLinear(nn.Module):
    ...
    def __init__(
        ...
        assert layer_type in {
            'input-hidden', 'hidden-hidden', 'hidden-output', 'input-output'
        }
        assert order in {'A', 'B'}
    ...
    def extra_repr(self):
        return 'in_features={}, out_features={}, bias={}, type={}, order={}'.format(
            self.in_features, self.out_features, self.bias is not None, self.layer_type,
            self.order
        )
...
class DownShiftConv2d(Conv2dWeightNorm):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), bias=True):
        ...
        self.shift_padding = (
            (kernel_size[1] - 1) // 2, (kernel_size[1] - 1) // 2, kernel_size[0] - 1, 0
        )
...
class DownShiftConvTranspose2d(ConvTranspose2dWeightNorm):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), bias=True):
        ...
        super(DownShiftConvTranspose2d, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride=stride,
    ...
    def forward(self, input):
        ...
        size = output.size()
        return output[:, :, :(size[2] - self.padding_reduce[0]
                             ), self.padding_reduce[1]:(size[3] - self.padding_reduce[1])]
...
class DownRightShiftConvTranspose2d(DownShiftConvTranspose2d):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), bias=True):
        super(DownRightShiftConvTranspose2d, self).__init__(
            in_channels, out_channels, kernel_size, stride=stride, bias=bias
        )
