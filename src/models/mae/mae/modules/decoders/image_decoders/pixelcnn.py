...
from ...networks.weight_norm import LinearWeightNorm, Conv2dWeightNorm
from ...networks.auto_regressives.pixelcnn import PixelCNN
...
class PixelCNNDecoderBinaryImage28x28(BinaryImageDecoder):
    ...
    def __init__(self, nz, mode, ngpu=1):
        ...
        self.core = nn.Sequential(
            PixelCNN(
                self.nc + self.fm_latent, hidden_channels, len(kernal_sizes),
        ...
        if ngpu > 1:
            self.z_transform = nn.DataParallel(
                self.z_transform, device_ids=list(range(ngpu))
            )
    ...
    def decode(self, z, random_sample):
        ...
        for i in range(H):
            for j in range(W):
                ...
                # [batch, nc]
                img[:, :self.nc, i, j] = torch.bernoulli(
                    recon_img[:, :, i, j]
                ) if random_sample else recon_img[:, :, i, j].ge(0.5).float()
    ...
    def reconstruct_error(self, x, z):
        ...
        BCE = (recon_x + eps).log() * x_flat.unsqueeze(1) + (1.0 - recon_x + eps).log(
        ) * (1. - x_flat).unsqueeze(1)
        # [batch, nsamples]
        return BCE.sum(dim=2) * -1.0

    @overrides
    def initialize(self, x, z, init_scale=1.0):
        z_transform = self.z_transform.module if isinstance(
            self.z_transform, nn.DataParallel
        ) else self.z_transform
