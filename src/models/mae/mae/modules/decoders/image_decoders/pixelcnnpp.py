...
from ...networks.weight_norm import Conv2dWeightNorm, ConvTranspose2dWeightNorm
from ...networks.auto_regressives.pixelcnnpp import PixelCNNPP
    ...
    def __init__(
        ...
        for i in range(levels - 1):
            blocks.append(
                (
                    'level%d' % i,
                    ConvTranspose2dWeightNorm(
                        in_channels, in_channels // 2, 3, 2, 1, 1, bias=True
                    )
                )
            )
...
class PixelCNNPPDecoderColorImage32x32(ColorImageDecoder):
    ...
    """
    def __init__(
        self, z_channels, h_channels, nmix, dropout=0., activation='elu', ngpu=1
    ):
        """
    ...
    def decode(self, z, random_sample):
        ...
        for i in range(H):
            for j in range(W):
                ...
                # [batch, nc, H, W]
                new_x = sample_from_discretized_mix_logistic(
                    mu, log_scale, coeffs, logit_probs, random_sample
                )
                x[:, :, i, j] = new_x[:, :, i, j]
        return x, None
...
class PixelCNNPPDecoderColorImage64x64(PixelCNNPPDecoderColorImage32x32):
    ...
    def __init__(
        ...
        Args:
            ...
            ngpu: number of gpus to use
        """
        super(PixelCNNPPDecoderColorImage64x64, self).__init__(
            z_channels,
            h_channels,
            nmix=nmix,
            dropout=dropout,
