...
class ColorImageDecoder(Decoder):
    ...
    def execute(self, z, x):
        ...
        log_scale = output[:, (self.nc + 1) * nmix:(self.nc * 2 + 1) *
                           nmix].view(batch, nmix, self.nc, H, W)
        coeffs = output[:, (self.nc * 2 + 1) * nmix:(self.nc * 2 + 4) *
                        nmix].view(batch, nmix, self.nc, H, W)

        return mu, log_scale.clamp(min=-7.), F.log_softmax(logit_probs,
                                                           dim=1), coeffs.tanh()
    ...
    def reconstruct_error(self, x, z):
        ...
        # [batch * nsamples]
        index = torch.cat([x.new_ones(nsampels) * i for i in range(batch_size)],
                          dim=0).long()
        # [batch * nsamples, nc, 32, 32]
        x_expand = x.index_select(0, index)

        # [batch * nsamples, mix, x_shape]
        mu, log_scale, logit_probs, coeffs = self.execute(
            z.view(batch_size * nsampels, *z_size[2:]), x_expand
        )
        # [batch, nsamples, mix, x_shape]
        ...
        return discretized_mix_logistic_loss(
            x_reshape, mu, log_scale, coeffs, bin_size, lower, upper, logit_probs
        )
