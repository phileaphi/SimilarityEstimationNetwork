...
parser.add_argument(
    '--data', choices=['mnist', 'omniglot'], help='data set', required=True
)
parser.add_argument(
    '--batch-size',
    type=int,
...
parser.add_argument(
    '--seed', type=int, default=524287, metavar='S', help='random seed (default: 1)'
)
parser.add_argument(
    '--log-interval',
    type=int,
...
parser.add_argument(
    '--opt', choices=['adam', 'adamax'], help='optimization method', default='adam'
)
...
parser.add_argument(
    ...
    default=0.0,
    metavar='N',
    help='free bits used in training.'
)
parser.add_argument(
    '--polyak',
    type=float,
    default=0.999,
    help=  # noqa
    'Exponential decay rate of the sum of previous model iterates during Polyak averaging'
)
parser.add_argument(
    '--schedule', type=int, default=20, help='schedule for learning rate decay'
)
...
def train(epoch):
    print(
        'Epoch: %d (lr=%.6f (%s), patient=%d (%d), decay=%d (%d))' %
        (epoch, lr, opt, patient, schedule, decay, max_decay)
    )
    mae.train()
    recon_loss = 0
    kl_loss = 0
    ...
    for batch_idx, (data, _) in enumerate(
        iterate_minibatches(train_data, train_index, args.batch_size, True)
    ):
        binary_data = binarize_image(data).to(device)

        batch_size = len(binary_data)
        optimizer.zero_grad()
        loss, recon, kl, pkl_m, pkl_s, loss_pkl_mean, loss_pkl_std = mae.loss(
            binary_data, nsamples=training_k, eta=eta, gamma=gamma, free_bits=free_bits
        )
        loss.backward()
        ...
        if batch_idx % args.log_interval == 0:
            ...
            log_info = '[{}/{} ({:.0f}%)] Loss: {:.2f} (recon: {:.2f}, kl: {:.2f}, pkl (mean, std): {:.2f}, {:.2f}, pkl_loss (mean, std): {:.2f}, {:.2f})'.format(
                batch_idx * batch_size, len(train_data),
                100. * num_insts / len(train_data), train_loss, recon_loss / num_insts,
                kl_loss / num_insts, pkl_mean / num_batches, pkl_std / num_batches,
    ...
    train_loss = recon_loss / num_insts + kl_loss / num_insts + pkl_mean_loss / num_batches + pkl_std_loss / num_batches
    print(
        'Average loss: {:.2f} (recon: {:.2f}, kl: {:.2f}, pkl (mean, std): {:.2f}, {:.2f}, pkl_loss (mean, std): {:.2f}, {:.2f})'
        .format(
            train_loss, recon_loss / num_insts, kl_loss / num_insts,
            pkl_mean / num_batches, pkl_std / num_batches, pkl_mean_loss / num_batches,
...
def eval(eval_data, eval_index):
    ...
    num_insts = 0
    num_batches = 0
    for i, (binary_data, _) in enumerate(
        iterate_minibatches(eval_data, eval_index, 512, False)
    ):
        binary_data = binary_data.to(device)

        batch_size = len(binary_data)
        loss, recon, kl, pkl_m, pkl_s, loss_pkl_mean, loss_pkl_std = mae_shadow.loss(
            binary_data, nsamples=test_k, eta=eta, gamma=gamma
        )
    ...
    print(
        'loss: {:.2f} (elbo: {:.2f} recon: {:.2f}, kl: {:.2f}, pkl (mean, std): {:.2f}, {:.2f}, pkl_loss (mean, std): {:.2f}, {:.2f})'
        .format(
            test_loss, test_elbo, recon_loss, kl_loss, pkl_mean, pkl_std, pkl_mean_loss,
            pkl_std_loss
        )
    )
...
def reconstruct():
    ...
    reorder_index = torch.from_numpy(
        np.array([[i + j * n for j in range(4)] for i in range(n)])
    ).view(-1)
...
def calc_nll():
    ...
    num_insts = 0
    for i, (binary_data, _) in enumerate(
        iterate_minibatches(test_binary_data, test_binary_index, 1, False)
    ):
        binary_data = binary_data.to(device)
    ...
    nll_iw /= num_insts
    print(
        'Test NLL: ELBO: {:.2f} (recon: {:.2f}, kl: {:.2f}), IW: {:.2f}, time: {:.2f}s'.
        format(nll_elbo, recon_err, kl_err, nll_iw, (time.time() - start_time))
    )
...
for epoch in range(1, args.epochs + 1):
    ...
    with torch.no_grad():
        loss, recon, kl, pkl_mean, pkl_std, pkl_mean_loss, pkl_std_loss = eval(
            val_binary_data, val_binary_index
        )
    elbo = recon + kl
    if elbo < best_elbo:
        patient = 0
    ...
    print(
        'Best: {:.2f} (elbo: {:.2f} recon: {:.2f}, kl: {:.2f}, pkl (mean, std): {:.2f}, {:.2f}, pkl_loss (mean, std): {:.2f}, {:.2f}), epoch: {}'
        .format(
            best_loss, best_elbo, best_recon, best_kl, best_pkl_mean, best_pkl_std,
            best_pkl_mean_loss, best_pkl_std_loss, best_epoch
        )
    )
    print(
        '============================================================================================================================'
